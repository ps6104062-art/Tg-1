...
# ── CONFIG ────────────────────────────────────────────────────────────────────
BOT_TOKEN        = os.getenv("BOT_TOKEN", "8986808212:AAGras7wZhKkILU9uotsOvuI3KMjm6t_4MY")
...
def kb_main(is_admin=False):
    rows = [
        [btn("🛒 Магазин аккаунтов", "shop_browse:0")],
...
async def cb_shop(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        f"{shop_header()}\n\n📦 Доступно: <b>{total}</b> | Стр. {page+1}/{pages}\n\n<i>Выбери аккаунт:</i>",
...
async def main():
    log.info("🚀 TG Manager Pro v2 запущен")
    await restore()
