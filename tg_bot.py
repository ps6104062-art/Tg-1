...
YOOMONEY_TOKEN   = ""
YOOMONEY_WALLET  = "+79770517190 юмани"
...
async def get_reg_date(client) -> str:
    """Получаем дату регистрации через @dateregbot"""
    try:
        ...
        if msgs:
            ...
            # Ищем паттерн типа "Регистрация: Июль 2026" или дату YYYY-MM
            m = re.search(r'(\w+\s+\d{4})', txt)
            if m:
                return m.group(1)  # "Июль 2026"
    except Exception as e:
...
async def auto_collect(phone: str, client) -> dict:
    ...
    # Точная дата через @creationdatebot
...
async def load_sess_phone(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "📋 <b>Отправь сессию в любом формате:</b>\n\n"
        ...
        "• <b>tdata папка</b> — отправь файлы из папки tdata (key_datas, D877F783D5D3EF8C)",
        reply_markup=kb_cancel(), parse_mode="HTML"
    )

async def _load_and_save(phone, session_str, msg_or_cb):
    """Общая функция загрузки сессии"""
    ...
    try:
        ...
        if not await cl.is_user_authorized(): raise Exception("Сессия невалидна или истекла")
        username, full_name = await get_profile(cl)
        db_save(ADMIN_ID, phone, cl, username, full_name)
        set_client(ADMIN_ID, phone, cl)
        attach_listener(cl, ADMIN_ID, phone)
...
async def load_sess_input(msg: Message, state: FSMContext):
    ...
    if msg.document:
        ...
        if fname.endswith(".json"):
            try:
                ...
                await w.delete()
                await _load_and_save(phone, sess, msg)
            except Exception as e:
                await w.edit_text(f"❌ Ошибка JSON: {e}", reply_markup=kb_back_menu())
            return

        # .session файл (Telethon SQLite) — сохраняем и читаем
        if fname.endswith(".session"):
            path = os.path.join(SESSIONS_DIR, f"upload_{phone.lstrip('+')}.session")
            with open(path, "wb") as f_: f_.write(raw)
            try:
                from telethon.sessions import SQLiteSession
                cl = TelegramClient(path.replace(".session",""), API_ID, API_HASH)
                await cl.connect()
                if not await cl.is_user_authorized(): raise Exception("Сессия невалидна")
                sess = StringSession.save(cl.session)
                username, full_name = await get_profile(cl)
                db_save(ADMIN_ID, phone, cl, username, full_name)
                set_client(ADMIN_ID, phone, cl)
                attach_listener(cl, ADMIN_ID, phone)
        ...
        # TXT или любой другой — читаем как строку
        try:
    ...
    # Текст — session string напрямую
    if msg.text:
        await _load_and_save(phone, msg.text.strip(), msg)
        return
...
async def ai_cat(cb: CallbackQuery, state: FSMContext):
    ...
    cl = get_client(ADMIN_ID, phone)
    if not cl:
        row = db.execute("SELECT session FROM accounts WHERE phone=? AND owner_id=?",(phone,ADMIN_ID)).fetchone()
