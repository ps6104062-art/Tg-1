...
async def get_spam_block(client) -> bool:
    """Проверяем спам-блок через @SpamBot"""
    try:
...
async def auto_collect(phone: str, client) -> dict:
    """Автоматически собираем все данные об аккаунте"""
    info = {
    ...
    try:
        ...
        if me:
            ...
            # Год регистрации через user id (приблизительно)
            uid = me.id
            if uid < 100_000:        info["year"] = 2013
            elif uid < 1_000_000:    info["year"] = 2014
            elif uid < 10_000_000:   info["year"] = 2015
            elif uid < 100_000_000:  info["year"] = 2016
            elif uid < 500_000_000:  info["year"] = 2017
            elif uid < 1_000_000_000:info["year"] = 2018
            elif uid < 1_500_000_000:info["year"] = 2019
            elif uid < 2_000_000_000:info["year"] = 2020
            elif uid < 4_000_000_000:info["year"] = 2021
            elif uid < 5_500_000_000:info["year"] = 2022
            elif uid < 7_000_000_000:info["year"] = 2023
            else:                    info["year"] = 2024
    except Exception as e:
...
async def ai_cat(cb: CallbackQuery, state: FSMContext):
    ...
    item_id = shop_add_item(
        ...
        spam_block  = info.get("spam_block", 0),
        has_premium = info.get("has_premium", 0),
        category    = cat,
    )
    await state.clear()
    await wait_msg.edit_text(
        ...
        f"📅 Год: {info.get('year') or '—'}\n"
        f"💎 Premium: {'✅' if info.get('has_premium') else '❌'}\n"
        f"🚫 Спам-блок: {'✅' if info.get('spam_block') else '❌'}\n"
        f"🔒 2FA: {'✅' if info.get('has_2fa') else '❌'}\n"
        f"📸 Фото: {'✅' if info.get('has_photo') else '❌'}\n"
        f"👥 Контактов: {info.get('contacts', 0)}\n"
        f"💬 Диалогов: {info.get('dialogs', 0)}\n"
